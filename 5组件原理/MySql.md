...
**//设置慢日志查询时间为**2s,超过2s视为慢查询,单位秒,默认为10s
...
- 一般是优化器选错索引导致,通过explain命令排查
- 客户端和MySQL内部连接数过少,大量请求会被阻塞等待

​		多建连接,MySQL端默认为100

对MySQL端:set global max_connections = 500;  //改为500
...
对应用侧连接:原理是基于tcp的长连接,有长连接池,通过ORM库进行**调大连接池数量**

- buffer pool 太小

​		查看缓存利用率(show status like 'innodb_buffer_pool_%'),一般命中率都在99%以上,低于才调大
